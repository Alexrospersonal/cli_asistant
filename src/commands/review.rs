...
pub async fn execute(path_buf: PathBuf) -> Result<(), Box<dyn Error>> {
    ...
    print_changing_to_cli(&original_file, &suggestion);
    ...
    save_file_to_tmp_folder(review_name, diff_as_string).await?;

    Ok(())
}

fn print_changing_to_cli(old: &str, new: &str) {
...
async fn save_file_to_tmp_folder(review_name: String, file: String) -> Result<(), Box<dyn Error>> {
...
fn mock_ai_suggestion(file: &str) -> String {
   let new_file: String = file.lines().map(|row| {
       let mut row = row.to_string();
       row.push_str("// suggested \n");
       row
   }).collect();
